...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size):
        super().__init__()
        self.token_embedding_table = nn.Embedding(vocab_size, vocab_size)# 92 x 92 size lookup table
        # each token directly reads off the logits(score) for the next token from a lookup table

    def forward(self, idx, targets=None):

        #idx(input indexes (xb)) and targets(yb) are both (Batch,Time) tensor of integers [taken when the model is called]
        logits = self.token_embedding_table(idx) #(B,T,C(channels, here vocab size i.e, 92))
...
model = BigramLanguageModel(vocab_size) #vocab size passed
m = model.to(device)
